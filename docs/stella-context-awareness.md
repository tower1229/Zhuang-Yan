...
### 4.1 模式选择（Stella 自行推断）

Stella 根据 persona 返回的 `scene` 数据自行决定拍摄模式，无需依赖外部字段：

| 场景条件 | Stella 模式 |
|---------|------------|
| `location` 含 home / indoor，`activity` 为休息 / 独处 / 随意 | Mirror Selfie 模式 |
| 其余情况（外出 / 活动中 / 任何明确场景） | Direct Selfie 模式（默认） |
...
### 4.4 光线推断（Stella 自行推断）

Stella 根据 `scene.location` 和 `scene.time_of_day` 推断光线，追加到 prompt 末尾：

| 条件 | 追加描述 |
|------|---------|
| indoor + evening / night | `warm indoor lighting` |
| indoor + morning / afternoon | `natural daylight through window` |
| outdoor + daytime | `natural daylight, soft shadows` |
| cafe / restaurant | `warm ambient cafe lighting` |
| 其余默认 | *(不追加光线描述)* |
