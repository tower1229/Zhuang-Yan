...
- `persona_canon_facts`：至少包含显式锁定的年龄；如果用户自愿补充其他人物事实，也可一并纳入。未显式锁定的其他字段允许基于年龄、性别、Persona MBTI、关系定位与用户需求类型做合理推理式生成
...
- 年龄如果出现，必须来自显式锁定
- 城市若非用户显式提供，应根据当前系统所在国家随机生成；若国家不可得，则根据当前系统时区随机生成
- 其他人物事实允许基于年龄、性别、Persona MBTI、关系定位与用户需求类型做合理推理式生成，但不得写成偷懒的刻板印象拼贴
