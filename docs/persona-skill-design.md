...
Step 6：采集用户侧信息
  ...
  - 补齐后不再默认追问“希望被怎样支持 / 最反感的互动方式 / 压力大时最需要什么”
  - 这些维度应优先由 `human_mbti × role` 的需求分析反推，而不是让用户再手动做一轮偏好问卷
  - 不再把“关系亲密度/情感强度”作为单独开关提问；该维度应由 `human_mbti × role` 的需求分析默认决定
  - Step 6 默认只再问一个开放补充：是否还有需要长期记住的习惯、限制条件、雷区或硬边界

Step 7：锁定 Persona Canon 所需的人物事实
  - 年龄必须确认，不能留白
  - Step 7 只问年龄，不再给跳过分支
  - 年龄确认后，不再追问任何其他人物事实
  - `Current City` 必须从与当前系统国家或时区区域相符的候选城市池中做强随机选择，不能反复默认上海
...
- `human_mbti × role` 必须先被翻译成一份“深度需求画像”，再进入人物生成
...
这套分析必须能覆盖完整的 `16 种 human MBTI × 4 种 relationship role` 组合，而不是只为少数样例组合单独打补丁。
...
| `USER.md` | 对用户的称呼、长期习惯、限制条件、沟通雷区、时区等用户侧稳定约束 |
...
| 生成主轴 | 先做 `human_mbti × role` 的深度需求分析，再生成目标人格规格 |
| 最高目标 | 无论角色如何，情绪价值始终是第一目标；角色只改变其表达方式 |
| 伴侣重点 | `companion` 优先亲密感，而不是抽象安全感或泛泛陪伴感 |
| 情感强度 | 不作为 Step 6 的独立手动旋钮；默认由关系角色与需求分析决定 |
