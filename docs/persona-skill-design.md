...
Step 7：锁定 Persona Canon 所需的人物事实
  - 默认只确认年龄
  - 先问用户是暂时留白，还是锁定年龄，还是在锁定年龄后自愿补充额外事实
  - 如果用户只锁定年龄，不继续追问城市、职业、家庭背景、兴趣等
  - 城市根据当前系统所在国家；如果国家不可得，则根据当前系统时区随机选定
  - 其他人物事实可由年龄、性别、Persona MBTI、关系定位与用户需求类型做合理推理式生成
  - 允许用户自愿补充额外事实；这些显式事实优先于系统生成结果
  - 生成的人物事实必须具体、连贯，但不能写成偷懒的 MBTI 刻板印象
