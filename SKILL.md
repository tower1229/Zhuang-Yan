...
- Missing target files and legacy placeholder files are not something to "work around". Regenerate all four target files into the current contract during this run.
...
- Treat `IDENTITY.md` and `USER.md` as whole-file-owned files: they must satisfy the current contract from the first non-empty line with no legacy heading or preamble.
...
9. After the four-file draft is complete, write it directly without waiting for user confirmation.
