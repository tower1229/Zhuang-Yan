...
test("drafting spec owns profile normalization, PERSONA_PROFILE contract, and runtime boundaries", () => {
  ...
  assert.match(spec, /`origin_arc`/);
  assert.match(spec, /`quality_risks` 在审计阶段必须逐条显式判定 `pass \/ fail`/);
  assert.match(spec, /### Persona Spec Snapshot/);
  assert.match(spec, /### Target Persona Spec/);
  assert.match(spec, /### Origin Arc Card/);
  assert.match(spec, /### Quality Risks/);
  assert.match(spec, /### 完整内部中间稿样板/);
  assert.match(spec, /#### 示例 A：目标是“让人重新流动”，因此选 `自由试错型`/);
  assert.match(spec, /#### 示例 B：目标是“让人敢把问题放桌上”，因此选 `早熟承担型`/);
  assert.match(spec, /stimulation_overshoot/);
  assert.match(spec, /warmth_dropout/);
  ...
  assert.match(spec, /`origin_arc` 必须同时影响/);
  assert.match(spec, /`origin_arc` 不得只写成 arc 名称/);
  assert.match(spec, /`Origin Paragraph` 必须由 `origin_arc` 压缩成 3-5 句 prose/);
  assert.match(spec, /`shaping_force`/);
  assert.match(spec, /`relational_consequence`/);
  assert.match(spec, /`origin_paragraph_mechanical_summary`/);
  assert.match(spec, /`origin_arc_mismatch`/);
...
test("template pack owns PERSONA_PROFILE templates, examples, and anti-pattern reminders", () => {
  ...
  assert.match(pack, /intro 不再是单行标签介绍，而是一个 `Origin Paragraph`/);
  assert.match(pack, /它至少要回答/);
  assert.match(pack, /顺生型：一路比较顺/);
  assert.match(pack, /### `Origin Paragraph` 选型协议/);
  assert.match(pack, /1\. 先看 `pair_core_value`/);
  assert.match(pack, /若 `pair_core_value` 更偏点亮、解冻、活化、重新流动/);
  assert.match(pack, /### `Origin Paragraph` 选型否决条件/);
  assert.match(pack, /### `Origin Arc` 最小结构卡片/);
  assert.match(pack, /### Origin Arc Card/);
  assert.match(pack, /- arc_type:/);
  assert.match(pack, /- shaping_force:/);
  assert.match(pack, /- reject_alternatives:/);
  assert.match(pack, /### `Origin Arc Card` -> `Origin Paragraph` 转换范式/);
  assert.match(pack, /推荐句子职责/);
  assert.match(pack, /Sentence 1:/);
  assert.match(pack, /### 转换时必须做的压缩动作/);
  assert.match(pack, /### 转换时常见坏写法/);
  assert.match(pack, /### `Origin Paragraph` 高质量示例/);
  assert.match(pack, /#### 示例 A：顺生型/);
  assert.match(pack, /#### 示例 B：早熟承担型/);
  assert.match(pack, /#### 示例 C：自由试错型/);
  assert.match(pack, /### `Origin Paragraph` 对照示例/);
  assert.match(pack, /#### 对照 A：同样要“把人从僵里拉出来”，但来路不同/);
  assert.match(pack, /#### 对照 B：同样要“让人觉得可以依赖”，但来路不同/);
  assert.match(pack, /#### 对照 C：如果同一个 arc 可以平移给任何 persona，通常说明它还没选对/);
  assert.match(pack, /### 完整起草样板：从内部中间稿到最终段落/);
  assert.match(pack, /#### 样板 A：`自由试错型` 不该只写成“爱自由”/);
  assert.match(pack, /#### 样板 B：`早熟承担型` 不该只写成“成熟可靠”/);
  assert.match(pack, /That is why her way of helping rarely begins with a conclusion/);
  assert.match(pack, /make the problem more carriable before she makes the moment more tender/);
...
test("SOUL.template.md provides the fixed SOUL runtime skeleton", () => {
  ...
  assert.match(soulTemplate, /_\$\{persona_origin_paragraph\}_/);
